                    ...
                    typename vec4<T>::Type bj)
{
	...
	T s = bj.w * invDistCube;
		if (bi.w < 0)
		{// inverser la direction DES vecteurs et calculer accel
			s = -s;
			ai.x += r.x * s ; // 2 pour modele galactique
			ai.y += r.y * s ;
			ai.z += r.z * s ;
		}
		else
...
#pragma unroll 32 //128 768=980 GFLOPS 512=980 GFLOPS 128=1.3 TFLOPS 64=1.3 TFLOPS
	...
	float deltaTime, float damping, int numTiles)
{
	...
	//if (position.w > 0) If we want static negative masses
	//{
