...
bool QATest = false;
int blockSize = 256;//256 original value
...
int flopsPerInteraction = 21;

char deviceName[100];

enum { M_VIEW = 0, M_MOVE };

int numBodies = 350200;
...
int activeDemo = 2;
//original 10 secondes
float demoTime = 1000000.0f; // ms
...
class NBodyDemo
{
    ...
    private:
                   ...
                   bool bUsePBO, bool useHostMem, bool useCpu)
        {
            ...
            m_hPos = new T[numBodies*4]; // Tableau de toutes les positions + masse
            m_hVel = new T[numBodies*4]; // tableau de toutes les velocités
        ...
        void _resetRenderer()
        {
            if (fp64)
            {
                float color[4] = { 0.4f, 0.8f, 0.1f, 1.0f}; //couleurs de base vert pour FP64
...
void finalize() //finalize() original modifé en ajoutant tableau Vel et Pos
{
    if (!useCpu)
    {
